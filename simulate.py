...
# Sample data
particles = [Particle(SIM_PARAMS, set_start=[-0.2, -0.2, -math.pi/2]), 
             Particle(SIM_PARAMS, set_start=[0.2, -0.2, -math.pi/2]),
             Particle(SIM_PARAMS, set_start=[0, 0.2, -math.pi/2])
            ]
time_steps = 3
...
for i in range(num_plots):
    scatter = axs[i].scatter(x[i*step_space], y[i*step_space])
    
    for j in range(len(particles)):
        dx = arrow_length * np.cos(theta[i*step_space][j])
        dy = arrow_length * np.sin(theta[i*step_space][j])
        axs[i].arrow(x[i*step_space][j], y[i*step_space][j], dx, dy, head_width=0.05, head_length=0.05, fc=arrow_color, ec=arrow_color)

    axs[i].set_title(f"Timestep {i*step_space} - Align_Corr: {align_corr[i*step_space]:.2f}")
    axs[i].set_xlim(-size, size)
    axs[i].set_ylim(-size, size)
    axs[i].grid(True)
